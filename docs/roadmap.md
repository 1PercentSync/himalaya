...
- **Compute Tile-Based 管线**：Projection+Culling → per-tile count / offset / scatter → per-tile local ordering → Tile Rendering（16×16 tile，前到后 alpha blend + early termination）；两次全局 RadixSort 路径仅作为已跑通原型，不作为最终优化主线

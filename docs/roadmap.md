...
- **Compute Tile-Based 管线**：Projection+Culling → Tile Entry Generation → Stable Radix Sort(depth) → Stable Radix Sort(tile-id) → Tile Range Build → Tile Rendering（16×16 tile，前到后 alpha blend + early termination）

...
init() 流程:
  ...
  3. 两组各自决定：命中 → read_ktx2 + upload，未命中 → compute + readback + write_ktx2
  4. 注册 bindless
...
| IBL 缓存 | 两组独立缓存（BRDF 固定 key + 3 cubemaps HDR hash），per-product staging buffer readback，切换 HDR 时只重算 cubemaps | 启动从分钟级降至秒级，切换 HDR 时 BRDF 免重算 |

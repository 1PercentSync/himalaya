...
**选择 A。** PCF 采样无边界溢出问题（每 layer 独立纹理空间），采样代码简洁（layer index = cascade index），与 `m1-interfaces.md` 预定义的 `sampler2DArrayShadow` 一致。Atlas 的灵活分辨率优势在实践中不常用（PSSM 分割已处理距离分布）。M3 Shadow Atlas 是面向多种光源的统一管理系统，与 CSM texture array 在管理复杂度上差距巨大，无论现在选哪种方案 M3 都需要另建 atlas 管理系统。

### Shadow Map 参数

| 参数 | 值 | 可调性 |
|------|-----|-------|
| Cascade 数量 | 4（默认） | 运行时可调（2/3/4），沿用 MSAA 切换模式 |
| 每 cascade 分辨率 | 2048² | 运行时可调（512/1024/2048/4096），与 cascade 数量共享同一变更 handler |
...
**选择：先暴力全画验证正确性，再泛化 Culling 模块**

| 阶段 | 策略 |
|------|------|
| Step 2-5 | 暴力全画全部场景物体，GPU 自动裁剪 frustum 外几何。Sponza 规模下 ~2000 depth-only draw calls 无性能问题 |
| Step 6 | 泛化 Culling 模块为通用 frustum 剔除（`Frustum` 结构体 + `extract_frustum(mat4)` + `cull_against_frustum(instances, frustum)`），ShadowPass 每个 cascade 调用。现有相机 frustum cull 同步迁移到通用接口 |

Shadow pass 的输入不是相机剔除结果（会遗漏 off-screen shadow caster），而是全部场景物体。Step 6 的 per-cascade 光空间剔除从全部物体中过滤。
...
| Shadow 质量 | PCF + lerp cascade blend（M2 切 dithering）+ triple bias（constant+slope 硬件 + normal offset shader）+ alpha mask 阴影 | 性价比最优组合 |

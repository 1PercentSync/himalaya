...
| `on_resize()` | 创建/重建 resolution-dependent 资源 | init 时 setup 后调用，resize 时单独调用 |
...
**各 pass 方法集允许不统一**，但同功能方法保持同名。例如 SkyboxPass 方法集为 `setup()` / `record()` / `destroy()`，不需要 `on_resize()` 和 `on_sample_count_changed()`——SkyboxPass 无分辨率相关私有资源，pipeline `rasterizationSamples` 固定为 1。ForwardPass 和 DepthPrePass 拥有完整五方法集。

#### MSAA Pipeline 重建

MSAA 切换需要重建 pipeline（`rasterizationSamples` 烘焙在 pipeline 中），但 `setup()` 是一次性初始化、`on_resize()` 语义上是分辨率变化，都不适合承载此职责。
...
**选择 A。** 命名遵循 `on_resize()` 的"事件通知"模式——方法名描述"发生了什么"（sample count 变了），pass 内部决定如何响应（重建 pipeline）。排除 B：语义模糊，不关心 MSAA 的 pass（如 TonemappingPass）也要接收不属于自己的配置变化。排除 C：`setup()` 还做了编译 shader、存储服务指针等工作，MSAA 切换时完全不需要重做。
...
Pass 的 pipeline 创建逻辑抽为私有方法 `create_pipelines()`，`setup()` 调用它完成初始化。此设计为未来热重载预留结构——引入 `rebuild_pipelines()` 公开方法时直接调用已有的 `create_pipelines()`，无需重构。

```cpp
class ForwardPass {
public:
    void setup(..., uint32_t sample_count);   // 调用 create_pipelines()
    void on_sample_count_changed(uint32_t);   // 调用 create_pipelines()
    // ...
private:
    void create_pipelines(uint32_t sample_count);  // setup 和 MSAA 切换共用
};
```
...
**选择：新增 cascade_index，68 → 72 bytes**

Shadow VS 需要知道当前渲染的 cascade index 以从 GlobalUBO 读取对应的 `cascade_view_proj[cascade_index]`。cascade_index 作为 push constant 传递——每个 cascade 循环开头 push 一次，该 cascade 所有 draw call 共享。

72 bytes 仍在 128 bytes 最低保证内。Forward/PrePass 不读 `cascade_index`，所有 pipeline 共享同一 layout（72 bytes push constant range）。

**阶段六演进**：per-instance SSBO 迁移后 push constant 缩减为 `{instance_id(4), cascade_index(4)}` = 8 bytes。cascade_index 不是 per-instance 数据，留在 push constant 中。
...
```cpp
// Set 0, Binding 3
struct GPUInstanceData {              // 80 bytes, std430
    mat4 model;                       // 64 bytes
...
Shader 通过 `gl_InstanceIndex` 索引（`vkCmdDrawIndexed` 的 `firstInstance` 参数设置起始偏移）。Push constant 从 72 bytes 缩减为 4 bytes（仅 `cascade_index`，shadow pass 用）。
...
**InstanceBuffer 管理**：CpuToGpu 内存，per-frame buffer（2 frames in flight），固定大小（kMaxInstances=65536 × 80 bytes = 5 MB），每帧 memcpy 覆写。超出上限时 warn 并丢弃剩余 group。
...
bc7enc 是 Rich Geldreich 的 BC1-7 编码器，MIT 许可，bc7enc.h/cpp + rgbcx.h 三个文件直接编译进项目。纯 C++ 实现，无 ISPC 依赖。

排除方案：
- **ISPCTextureCompressor**：更快，但需要 ISPC 编译器，无 CMake/vcpkg 集成，构建配置复杂
- **bc7enc_rdo**（ISPC 后端）：质量更高但依赖 ISPC，bc7enc 纯 C++ 后端质量足够（缓存后首次压缩速度不敏感）
...
```
首次: PNG → RGBA8 → CPU mip → BC 压缩 → 上传 GPU + write_ktx2() 写缓存
缓存: read_ktx2() → upload_image_all_levels() 直接上传 BC 数据（零 transcode）
```
...
**缓存 key**：源图像数据的内容哈希。文件变化自动重新压缩。首次加载需读取 PNG 计算哈希，但这是首次开销，后续命中时只需比较哈希。

### Mip 生成

**选择：CPU 端 stb_image_resize2 替代 GPU vkCmdBlitImage**

BC 纹理不支持 `vkCmdBlitImage`（GPU 不能逐像素写入 BC block）。CPU 端逐级缩放后每级独立压缩。stb_image_resize2 是 stb 系列头文件库，已有 stb_image 基础，集成成本为零。
...
多张纹理同时压缩，利用多核 CPU。首次加载数百张纹理的压缩时间从分钟级降到可接受范围。实现简单，无需 block 级并行。
...
**DFD（Data Format Descriptor）处理**：KTX2 规范要求每个文件包含有效 DFD。对 6 种支持格式各预建一个 `static const uint8_t[]` 硬编码 blob（内容由 Khronos Data Format Specification 确定）。读取时不解析 DFD——仅用 header 的 `vkFormat` 字段识别格式。DFD 的存在是为了让外部工具能正确识别文件格式。

**文件布局**（按 KTX2 规范）：identifier → header → index section → level index → DFD → mip data（从最小 mip 到最大 mip）。mip 数据间按 `lcm(block_bytes, 4)` 对齐。

**读取路径**：读整个文件到内存 blob → 校验 identifier + header → 解析 level index → 返回 `Ktx2Data`（持有 blob + 每级 offset/size 索引），消费者直接通过 offset 索引 blob 中的 mip 数据传给 `upload_image_all_levels()`。
...
```
首次: HDR → GPU compute → readback 4 个产物 → 写 KTX2 缓存 → 注册 bindless
缓存: 读 KTX2 → 直接上传 GPU → 注册 bindless（跳过全部 compute）
```
...
| 产物 | 格式 | 大小 | cache key |
|------|------|------|-----------|
| Skybox cubemap | R16G16B16A16F, 2048², 6 faces + mips | ~256 MB | HDR 内容哈希 |
| Irradiance | R11G11B10F, 32², 6 faces | ~0.03 MB | HDR 内容哈希 |
| Prefiltered | R16G16B16A16F, 512², 6 faces + mips | ~16 MB | HDR 内容哈希 |
| BRDF LUT | R16G16_UNORM, 256² | ~0.25 MB | 固定 key（与 HDR 无关） |

KTX2 天然支持 cubemap 存储（`numFaces = 6` + mip chain），与纹理缓存共用自写 KTX2 读写模块。

**Readback 方案**：在 `end_immediate()` 之前，通过 staging buffer + `vkCmdCopyImageToBuffer` 一次性读回 4 个产物。immediate scope 内一次性开销，不影响运行时帧率。

**BRDF LUT 永久缓存**：BRDF Integration LUT 仅依赖 GGX BRDF 公式，与输入 HDR 无关。使用固定 cache key，更换 HDR 时不重算。
...
| IBL 管线 | GPU 运行时预计算，Framework 层，equirect .hdr 输入，IBL 自行加载并自管理全部资源，config.json 配置路径 | 一次性开销，M2 Reflection Probes 复用 |
...
| Instancing | CPU 侧 mesh_id 分组 + per-instance SSBO（Set 0 Binding 3, 80 bytes/instance）+ instanced draw，push constant 72→4 bytes，透明物体例外 | draw call 显著减少，提前合并阶段六 SSBO 迁移和 M3 Instancing |
...
| 纹理压缩 | BC7（通用）+ BC5（法线），bc7enc 纯 C++ 编码，首次压缩缓存到 `%TEMP%`，缓存命中零 transcode 直接上传，CPU mip 生成 + 纹理级并行压缩 | RGBA8 → BC 约 4:1 压缩比，GPU 原生解码零开销 |
| KTX2 读写 | 自写最小模块（6 种格式，DFD 硬编码），不依赖 libktx。纹理和 IBL 缓存共用 | libktx vcpkg 构建失败，实际需求远小于 libktx 功能集 |
...
| IBL 缓存 | GPU 预计算后 readback 缓存为 KTX2，后续直接加载跳过 compute，BRDF LUT 固定 key 永久缓存 | 启动从分钟级降至秒级 |

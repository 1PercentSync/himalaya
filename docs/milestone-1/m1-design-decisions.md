...
<!-- Debug Utils、资源引用方式、READ_WRITE 语义、RG 接管范围、ImGui 作为 RG Pass（阶段二）已迁移 -->
...
<!-- Swapchain Image 导入、RG 与 ResourceManager 关系、Barrier Aspect 推导、Framework 层 Vulkan 类型使用（阶段二）已迁移 -->

---

<!-- 资源管理与描述符（Bindless + Push Constant、Descriptor Set 三层架构、Pool 分离、Set 0 策略、DescriptorManager 职责、Set Layout 与 Pipeline Layout、资源 Debug Name、Bindless 纹理数组、Set 0 Layout、Set 1 Layout）（阶段二）已迁移至 archive/m1-phase2-decisions.md -->
...
<!-- DescriptorManager 职责、Set Layout 与 Pipeline Layout、资源 Debug Name、Bindless 纹理数组、Set 0 Layout、Set 1 Layout（阶段二）已迁移 -->
...
<!-- 场景数据接口（阶段二）已迁移至 archive/m1-phase2-decisions.md -->
...
<!-- 配置与调参系统、深度缓冲与精度（D32Sfloat + Reverse-Z）（阶段二）已迁移至 archive/m1-phase2-decisions.md -->
...
<!-- Sampler 管理、Default 纹理、纹理格式选择、GPU 上传策略（含 Image 上传/Staging Buffer）、场景加载与错误处理（阶段二）已迁移至 archive/m1-phase2-decisions.md -->

<!-- 资源句柄设计、对象生命周期、帧同步、错误处理与调试（阶段一）已迁移至 archive/m1-phase1-decisions.md -->
...
<!-- 阶段二条目（Render Graph、资源/描述符、顶点格式、材质系统、Shader 系统、场景数据、调参、深度缓冲、资源 Debug Name、Sampler）已迁移至 archive/m1-phase2-decisions.md -->
...
| Shadow 质量 | PCF + lerp cascade blend + dual bias（slope 硬件 + normal offset shader，constant 对 D32Sfloat 无效已移除）+ alpha mask 阴影 | 性价比最优组合 |
...
| Distance fade 参数 | 独立 `distance_fade_width`（不复用 blend_width），当前默认 0.1 | 语义不同于 cascade blend，ShadowConfig/UBO 独立字段备 M2+ 分离，M1 DebugUI Blend Width 同时驱动两者 |

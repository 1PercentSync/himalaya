...
`setup()` 签名：`void setup(rhi::Context&, rhi::ResourceManager&, rhi::DescriptorManager&, rhi::ShaderCompiler&)`。Pass 在 setup 中存储这些服务的指针（`ctx_`、`rm_`、`dm_`、`sc_`），lambda 通过 `this` 访问。ShaderCompiler 是 Layer 0 类型，Pass（Layer 2）依赖它不违反层级；热重载时 Pass 可自行重新编译 shader 重建 pipeline，Renderer 不需要介入。

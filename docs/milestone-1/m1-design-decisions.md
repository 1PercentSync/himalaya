...
长期不变的服务引用（ResourceManager、DescriptorManager、ShaderCompiler 等）由 Pass 在 `setup()` 时存储指针，不放入 FrameContext。FrameContext 是纯每帧数据，不做 service locator。

详细定义待确认，见 `m1-phase3-open-questions.md`。
...
| 方法 | 职责 | 调用时机 |
|------|------|----------|
| `setup()` | Pipeline 创建等不依赖分辨率的一次性工作 | 初始化 |

...
**Distance fade 与 cascade blend 参数分离**：`distance_fade_width`（阴影远端淡出区间占 `max_distance` 的比例）独立于 `blend_width`（cascade 间过渡区间）。两者语义不同——blend_width 控制相邻 cascade 质量差异的平滑过渡，distance_fade_width 控制最远 cascade 边缘到无阴影的淡出。当前默认值均为 0.1，但独立参数允许后续分别调参。
...
- **Blocker Search 搜索半径为常数**：正交投影无透视缩放，搜索半径直接等于 `LIGHT_SIZE_UV`
...
**选择：固定 16 blocker + 25 PCF**

PCSS 报告引用的"shipped titles"标配预算。25 PCF tap × 2×2 硬件双线性比较 = 100 次有效深度比较，M1 无 TAA 的单帧质量足够。

不做运行时可配置——动态循环导致 warp 内分支发散，且 M1 无需运行时调采样数。M2 加 TAA 后可降低采样数（改常量即可）。
...
方案 B 将 NDC depth 通过 per-cascade 正交投影参数（`cascade_ortho_width`）转换为世界空间距离后再计算半影宽度，同一遮挡距离在所有 cascade 中产生完全一致的半影估计。实现代价仅为 shader 中几次乘除 + 一个额外的 `vec4` UBO 字段。

#### GlobalUBO PCSS 字段

扩展现有 GlobalUBO（656 → 704 bytes），不新增 binding（理由同「GlobalUBO Shadow 数据布局」）。
...
| `cascade_light_size_uv` | `vec4` | per-cascade `LIGHT_SIZE_UV`（Blocker Search 搜索半径） |
| `cascade_ortho_width` | `vec4` | per-cascade 正交投影世界空间宽度（NDC→世界空间换算） |
...
| Distance fade 参数 | 独立 `distance_fade_width`（不复用 blend_width），当前默认 0.1 | 语义不同于 cascade blend，独立可调 |

...
同一 mesh 的所有 instance 共享同一 material（glTF primitive 级别绑定），pipeline state 完全一致。按 mesh_id 分组后，每组一次 `vkCmdDrawIndexed(instanceCount=N)`，draw call 显著减少（unique mesh 上限，实际更少因为 frustum cull）。
...
2. Renderer 按 mesh_id 排序 → 构建 MeshDrawGroup + 填充 InstanceBuffer
3. Pass 迭代 draw groups：`set_cull_mode` → `bind_vb/ib` → `draw_indexed(instanceCount=N, firstInstance=offset)`

**InstanceBuffer 管理**：CpuToGpu 内存，per-frame buffer（2 frames in flight），固定大小（场景最大 instance 数 × 80 bytes ≈ 230 KB），每帧 memcpy 覆写。
...
```
首次: PNG → RGBA8 → CPU mip → BC 压缩 → 上传 GPU + 写 KTX2 缓存
缓存: 读 KTX2 → 直接上传 BC 数据（零 transcode）
```

KTX2 是 Khronos 官方纹理容器格式，天然支持 BC 格式 + 多 mip 存储。libktx 仅用于 KTX2 文件 I/O，Vulkan 上传走现有 ResourceManager。
...
KTX2 天然支持 cubemap 存储（`numFaces = 6` + mip chain），与纹理缓存共用 libktx。
...
| IBL 管线 | GPU 运行时预计算，Framework 层，equirect .hdr 输入，IBL 自行加载并自管理全部资源，CLI11 命令行参数 | 一次性开销，M2 Reflection Probes 复用 |
...
| 纹理压缩 | BC7（通用）+ BC5（法线），bc7enc 纯 C++ 编码 + libktx KTX2 容器，首次压缩缓存到 `%TEMP%`，缓存命中零 transcode 直接上传，CPU mip 生成 + 纹理级并行压缩 | RGBA8 → BC 约 4:1 压缩比，GPU 原生解码零开销 |

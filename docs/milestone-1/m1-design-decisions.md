...
当前 GlobalUBO 304 bytes，阶段三新增 IBL 字段（4× uint32）后约 320 bytes。M1 全部完成后预估仍远小于 1KB，远在 UBO 最小保证大小 16KB（Vulkan spec）之下。暂不拆分。
...
| B/C. Set 1 新增 binding（bindless cubemap 数组） | 一步到位，Reflection Probes 复用 |

**选择 B/C。** 阶段三只有 2-3 个 cubemap（irradiance + prefiltered），但 Set 1 bindless 数组为 M1 阶段六 Reflection Probes 提供零改动的扩展路径。
...
**D32Sfloat**（32-bit 浮点深度，无 stencil）。M1 不需要 stencil 操作，纯深度格式能获得更好的深度精度。对后续 CSM 和屏幕空间效果（SSAO、contact shadows）的深度采样更有利。MSAA depth buffer 沿用 D32Sfloat 不变（Reverse-Z 需要浮点深度才能发挥精度分布优势；D24UnormS8 是定点格式且浪费 8-bit stencil，带宽与 D32Sfloat 相同）。
...
| 格式 | 大小 | 精度 | Alpha |
|------|------|------|-------|
| R11G11B10F | 4B/px | 6-bit mantissa (R/G), 5-bit (B) | 无 |
| R16G16B16A16F | 8B/px | 10-bit mantissa | 有 |
...
**选择 C。** 三个预计算总共几十到几百毫秒（一次性），不影响运行时性能。GPU compute shader 在 `begin_immediate()` / `end_immediate()` scope 内执行。
...
| 产物 | 分辨率 | 格式 | 内存 | 说明 |
|------|--------|------|------|------|
| Irradiance Map | 32×32 per face | R11G11B10F | ~48 KB | 极低频信号 |
| Prefiltered Env Map | 256×256 per face (含 mip chain) | R16G16B16A16F | ~24 MB | 高精度避免镜面反射 banding |
| BRDF Integration LUT | 256×256 | R16G16_UNORM | ~256 KB | 值域 [0,1]，与环境无关，所有环境共享 |

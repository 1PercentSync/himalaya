...
| Pass 抽象 | 阶段三引入具体类（阶段二用 lambda 回调），format 硬编码（Swapchain 例外：`setup()` 传入），`on_sample_count_changed()` 重建 MSAA pipeline，各 pass 方法集允许不统一，`create_pipelines()` 私有方法预留热重载 | 有实际经验后再提取，阶段四评估虚基类 |

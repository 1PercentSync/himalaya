...
`setup()` 签名因 pass 而异。MSAA 相关 pass（ForwardPass、DepthPrePass、SkyboxPass）接收 `sample_count`：
...
非 MSAA pass（TonemappingPass）不接收 `sample_count`：
...
只有 pipeline 中 `rasterizationSamples` 依赖 sample count 的 pass 需要实现此方法——ForwardPass、DepthPrePass、SkyboxPass。TonemappingPass（始终 1x，处理 resolved 产物）不需要。

#### Attachment Format 硬编码

Pipeline 创建时需要知道 color/depth attachment 的 `VkFormat`，但这些格式在 M1 期间全部是设计决策确定的常量：

| 格式 | 值 | 使用者 |
|------|------|--------|
| HDR color | `R16G16B16A16_SFLOAT` | ForwardPass, SkyboxPass |
| Depth | `D32_SFLOAT` | ForwardPass, DepthPrePass, SkyboxPass |
| Normal | `R10G10B10A2_UNORM` | DepthPrePass |
| Swapchain | `B8G8R8A8_SRGB` | TonemappingPass |

**选择：Pass 内部硬编码。** `setup()` 不传入 format 参数。M1 期间这些格式不会变化，传参增加接口复杂度但无实际收益。如果 M2 需要不同格式，改几行常量即可。这与"具体类，不用虚函数"的决策一致——每个 pass 完全了解自己的渲染配置，不需要外部告知。
...
阶段二 GlobalUBO 304 bytes。阶段三新增 IBL 字段（4× uint = 16 bytes）和 debug 字段（1× uint），总计 320 bytes（20 × 16，满足 std140 struct 对齐）：
...
void Renderer::handle_msaa_change(uint32_t new_sample_count) {
    ...
    skybox_pass_.on_sample_count_changed(new_sample_count);
    // tonemapping_pass_ 不受影响——始终 1x
}
```
...
GlobalUBO 新增 IBL 字段（**Step 6 添加**，Step 7 shader 直接消费）：
...
字段和 `bindings.glsl` 布局是一体的（C++ struct 必须与 GLSL uniform block 严格匹配），Step 6 产出 IBL 资源后立即更新 UBO 布局，避免 bindless index 无处存放的中间状态。Step 7 纯粹关注 shader 内的光照计算代码。
...
- **Shader**：复用 `fullscreen.vert` + 新增 `skybox.frag`（cubemap 方向采样）
...
| Pass 抽象 | 阶段三引入具体类（阶段二用 lambda 回调），format 硬编码，`on_sample_count_changed()` 重建 MSAA pipeline | 有实际经验后再提取，阶段四评估虚基类 |
| Skybox | 独立 RG pass（resolved 1x，GREATER_OR_EQUAL depth test），Step 6 引入 | M2 Bruneton 替换 |
...
| IBL 管线 | GPU 运行时预计算，Framework 层，equirect .hdr 输入 | 一次性开销，M2 Reflection Probes 复用 |

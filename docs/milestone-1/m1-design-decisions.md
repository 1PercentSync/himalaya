...
| Temporal 数据 | RG managed temporal（阶段五引入：`temporal=true` + `get_history_image()`，内部 double buffer + swap） | 一次建设，AO/SSR/SSGI 复用 |
| 调参 | ImGui + 配置结构体 | 能调就行 |
...
| AO 集成 | `ssao × material_ao` 乘法复合 + multi-bounce 色彩补偿（Jimenez 2016），仅调制间接光（IBL diffuse + specular） | 物理合理，浅色表面不过暗 |
| Specular Occlusion | GTAO 直接计算标量 SO（方案 B1：读 roughness buffer + 重建 R，per-direction 评估 specular cone 与 horizon 重叠） | 精度最高（vs bent normal 有损中间表示），AO 纹理仅 RG8（vs RGBA8） |
...
| Temporal 基础设施 | RG managed temporal（`temporal=true` + `get_history_image()`），per-frame Set 2（2 份对应 2 frames in flight） | RG 管理 double buffer 避免各 pass 重复代码，per-frame Set 2 解决 temporal binding 帧间竞争 |
| Compute pass 绑定 | Set 3 push descriptors（`push_storage_image` / `push_sampled_image` CommandBuffer helpers） | Temporal swap 天然解决，无 descriptor set 生命周期管理，IBL 已有基础设施 |
...
| Contact Shadows 多光源 | Per-light dispatch + per-light R8 输出，shader 接受光方向参数 | M1 一盏光 = 1 dispatch，M2 自然扩展 |

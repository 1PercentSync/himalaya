...
Shader 文件：`skybox.vert`（方向计算 + depth = 0.0）+ `skybox.frag`（cubemap 采样）。
...
| Pass 抽象 | 阶段三引入具体类（阶段二用 lambda 回调），format 硬编码，`on_sample_count_changed()` 重建 MSAA pipeline，各 pass 方法集允许不统一 | 有实际经验后再提取，阶段四评估虚基类 |
...
| IBL 管线 | GPU 运行时预计算，Framework 层，equirect .hdr 输入，IBL 自行加载，CLI11 命令行参数 | 一次性开销，M2 Reflection Probes 复用 |

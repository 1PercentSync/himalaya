...
| 资源 / 描述符 | Bindless（2D 纹理 4096 + Cubemap 256）+ 传统 Descriptor Set（Set 0/2 per-frame × 2）+ Push Constant + DescriptorManager 集中管理 | Bindless 早期投资值得，Set 2 per-frame 解决 temporal 帧间竞争 |
...
| Temporal 基础设施 | RG managed temporal（`temporal=true` + `get_history_image()`），per-frame Set 2（2 份对应 2 frames in flight，temporal binding 每帧更新当前帧 copy） | RG 管理 double buffer 避免各 pass 重复代码，per-frame Set 2 解决 temporal binding 帧间竞争（单份 Set 2 在 2 frames in flight 下 update 违反 Vulkan spec） |

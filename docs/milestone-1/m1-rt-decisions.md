...
- **PrimaryPayload**（location 0），逐步演进：
  ...
  - Step 13（68B）：+ `float cone_spread`（Ray Cones 扩展角，跨 bounce 传递）
...
- Step 12（24B）：+ `emissive_light_count`（uint，0 = 跳过 NEE emissive）
- Step 13（28B）：+ `lod_bias`（float，Ray Cones LOD 偏移，默认 0.0）
...
- Set 0 binding 8：EmissiveAliasTable（SSBO，`{float prob, uint alias}` 8B/entry + header，`PARTIALLY_BOUND`）

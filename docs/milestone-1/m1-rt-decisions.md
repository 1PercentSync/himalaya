...
- **Cancel 中途取消**：已完成的 instance 文件保留，该角度完整性校验不通过 → 不出现在可用列表
...
Cache key 包含场景固有信息和光照环境信息，不包含烘焙质量参数（SPP、分辨率、PT 参数、grid spacing）。改质量参数 → 手动触发重新烘焙（覆盖已有缓存）。

- Lightmap: `content_hash(scene_file) + mesh_geometry_hash + instance_transform_hash + hdr_content_hash + ibl_rotation_int`
- Probe set: `content_hash(scene_file) + hdr_content_hash`（不含 probe position——位置是 bake 产物，存在 manifest 中）
...
- `lm_content_hash`：`hash(scene_file + geometry + transform + hdr)`
...
- `probe_set_hash`：`hash(scene_file + hdr)`

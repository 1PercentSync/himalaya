...
两种 payload：
- **PrimaryPayload**（location 0）：`vec3 color` + `vec3 next_origin` + `vec3 next_direction` + `vec3 throughput_update` + `float hit_distance`（miss 时 -1 终止）+ `float env_mis_weight`（BRDF 采样方向的 env MIS 权重，closesthit 预计算）
...
raygen shader 的 push constant 包含 PT 特有参数（16 字节）：`max_bounces`（uint）、`sample_count`（uint）、`frame_seed`（uint）、`blue_noise_index`（uint，blue noise 纹理在 bindless textures[] 中的索引）。
...
M1 方向光为 delta 分布（Dirac delta PDF），BRDF 采样永远无法命中方向光。NEE 方向光的 MIS 权重恒为 1，退化为纯 NEE 无权重直接贡献。pt_common.glsl 中定义 `mis_power_heuristic()` 函数仅服务于 env importance sampling 和未来 M2+ 的面积光。

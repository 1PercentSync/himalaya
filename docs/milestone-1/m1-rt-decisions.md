...
场景 AABB 内按固定间距（默认 1m）放置 3D 网格候选探针。对每个候选点向多个方向发射射线（compute shader 中使用 `rayQueryEXT`），大部分射线在极短距离内命中几何体的点判定为墙内/物体内部，剔除。
...
Cache key 包含场景固有信息和光照环境信息，不包含烘焙质量参数（SPP、分辨率、PT 参数）。改质量参数 → 手动触发重新烘焙（覆盖已有缓存）。

- Lightmap: `content_hash(scene_file) + mesh_geometry_hash + instance_transform_hash + hdr_content_hash + ibl_rotation_int`
- Probe: `content_hash(scene_file) + probe_position_hash + hdr_content_hash + ibl_rotation_int`

`ibl_rotation_int` 为整数度数（0-359），bake 时将当前浮点旋转角 round 到最近整数。

场景加载时用相同输入重建 cache key 定位文件。场景几何变化、HDR 更换、IBL 旋转角度变化均导致 hash 变化 → 缓存自动失效。

#### 文件命名

`<content_hash>_rot<NNN>.ktx2`

- `content_hash`：除 rotation 以外的 cache key 部分的 hash
- `NNN`：零填充三位整数度数（000-359）

同一 scene+HDR 组合下可有多个旋转角度的 bake 并存。Phase 8 按 `<同一 content_hash>_rot*.ktx2` 模式扫描目录发现所有已 bake 角度。同一角度重新 bake 时覆盖旧文件。

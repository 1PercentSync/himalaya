...
Cache key 只含 glTF 固有信息，不包含烘焙参数（SPP、分辨率、PT 参数）。想改参数 → 手动触发重新烘焙（覆盖已有缓存）。

Lightmap cache key = `content_hash(scene_file) + mesh_geometry_hash + instance_transform_hash`。Probe cache key = `content_hash(scene_file) + probe_position_hash`。场景再次加载时用相同输入重建 cache key 自动命中缓存。几何或位置变化 → hash 变化 → 缓存失效 → 需重新烘焙。

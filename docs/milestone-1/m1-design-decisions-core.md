...
| 纹理 | Binding |
|------|---------|
| 材质纹理、BRDF LUT、Lightmap | binding 0（sampler2D[]） |
| IBL cubemap、Reflection Probes | binding 1（samplerCube[]） |
...
| 7 | `sampler2D` | bloom_texture | 八（预留） |
...
**变体演进**：无条件采样（阶段三，bindless + default 纹理）→ 动态分支（阶段六，per-object Lightmap）→ `#define` 编译变体（M2+，POM）→ Specialization Constants（可选）。

...
| Contact Shadows | Screen-space ray march + 深度自适应 thickness + 距离衰减 + 无 temporal | 确定性输出，shader 读 LightBuffer SSBO |
| Contact Shadows 多光源 | M1 单方向光单 dispatch 单 R8 | shader 读 `directional_lights[0]`，M2 需重构 |
...
**光方向来源**：Shader 直接读 LightBuffer SSBO（Set 0 Binding 1）`directional_lights[0].direction_and_intensity.xyz`。M1 单方向光场景下最简方案，无需 C++ 侧传递光方向。M2 多光源需重构（可改为 push constant 或循环 dispatch）。

**帧流程位置**：Contact Shadows 与 AO 管线零数据依赖（只共享 depth 只读访问，写入不同资源），录制在 AO 管线之后。不拆分 AO block，保持代码简洁。RG 不会在两者之间插入 barrier，GPU 可重叠执行。

**多光源**：M1 单方向光单 dispatch 单 R8。M2 多光源方案待定。

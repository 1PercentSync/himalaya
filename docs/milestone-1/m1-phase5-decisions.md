...
分阶段实施：Step 9 先用 Lagarde 近似公式（`saturate(pow(NdotV + ao, exp2(-16 * roughness - 1)) - 1 + ao)`，Lagarde & de Rousiers 2014）验证 AO 管线正确性（不需要 roughness buffer），Step 12 升级到 B1 后可直接对比精度差异。
...
**边缘权重 — 路径累乘对称**：5×5 kernel 中距离 >1 的样本，沿网格路径（先水平后垂直）累乘逐步边缘权重。例如 center→(2,0) 的权重 = edge(0,0↔1,0) × edge(1,0↔2,0)，其中 edge(A↔B) = w(A→B) × w(B→A)。这防止"穿隧"——即使两端深度相近，中间存在深度不连续处时权重仍为零，天然限制光晕扩散。
...
3. **邻域 clamp**：对当前帧输入（`ao_blurred`，空间降噪后）R 通道 3×3 `texelFetch` 采样取 min/max，将 history R 值 clamp 到 `[min, max]`（`texelFetch` 避免双线性插值缩小真实范围）。空间降噪后 clamp 范围更窄，stale history 被更积极拒绝 → 减少 ghosting。SO 通道（G）不做邻域 clamp，使用与 AO 相同的 blend factor

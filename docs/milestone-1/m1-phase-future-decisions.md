...
## ~~Set 1 — 持久纹理资产预留（阶段八构想，来自阶段二规划）~~ → 已在 Phase 8 实现

Lightmap 注册到 `textures[]`（binding 0），Probe cubemap 注册到 `cubemaps[]`（binding 1）。详见 `m1-rt-decisions.md` Phase 8 章节。
...
## ~~材质变体演进（阶段八/M2 构想，来自阶段三规划）~~ → Phase 8 部分已实现

Phase 8 采用动态分支（`feature_flags` bit 3 + per-instance `lightmap_index` 哨兵值检查），与构想一致。

| 阶段 | 策略 | 场景 |
|------|------|------|
| ~~阶段八~~ | ~~动态分支~~ → **已实现** | Lightmap 按 per-object 属性条件执行 |
...
## ~~Phase 8 间接光照模式（阶段八构想，来自阶段七 Baker 参数分类审查）~~ → 已在 Phase 8 实现

完整决策见 `m1-rt-decisions.md`「Phase 8 间接光照集成决策」章节。

主要变更相对于原始构想：
- `ibl_intensity` 重命名为 `indirect_intensity`（底层同一参数，UI 按模式显示不同文案）
- 模式控制提升为 Application 层 `IndirectLightingMode` enum（而非 feature_flag 级别），shader 端仍通过 `feature_flags` bit 3 接收
- 新增 BakeDataManager（framework 层）管理 bake 数据生命周期

---

## LP 模式根本修复：烘焙排除太阳直射（程序化天空阶段构想，来自阶段八点五 Step 5.1）

### 当前临时方案

Phase 8.5 Step 5.1 使用运行时合成公式调整（diffuse max + specular shadow 调制）消除双算。此方案在 HDR 太阳嵌入环境贴图的架构下足够，但不是物理正确的解。

### 根本修复方案

过渡到程序化天空后，太阳作为独立参数存在（方向、颜色、角径），不再嵌入 HDR。Baker 可在路径追踪中精确排除太阳直射：

- **首段 ray miss 太阳排除**：lightmap/probe baker 的 `trace_path` 中，bounce 0 miss 时检查 `dot(miss_dir, sun_dir) > cos(sun_angular_radius)` → 不累积该方向的辐射（或替换为天空背景色）
- 深层 bounce 的 NEE env 正常采样太阳 → 太阳→表面→texel 的间接反弹保留
- 烘焙结果 = 天空漫射 + 所有间接反弹（含太阳间接），不含太阳直射 texel 的分量

运行时改为标准相加合成：`baked_indirect + direct_light × shadow`。max/shadow 临时方案退役。

### 前置依赖

- 程序化天空系统（太阳参数独立于环境贴图）
- Baker push constants 扩展（传入 sun_dir + sun_cos_angle）

---

## 自动太阳检测（程序化天空阶段构想，来自阶段八点五 Step 5.1）

当前 HdrSun 模式的太阳坐标由用户手动在 equirect 图上指定（`hdr_sun_x/y`），存在偏差。

### 潜在方案

扫描 equirect HDR 找最亮区域：
- 简单版：全图最亮像素坐标
- 改进版：超过亮度阈值的像素区域加权质心（处理太阳跨多个像素的情况）
- 返回太阳中心坐标 + 角半径估算

### 用途

- 替代手动坐标设置，消除人工偏差
- 角半径可用于 PCSS 软度自动设置（见下一条）
- 程序化天空阶段作为 HDR→程序天空参数的自动提取工具

---

## PCSS 太阳软度自动设置（程序化天空阶段构想，来自阶段八点五 Step 5.1）

当前 PCSS 的 `light_size` 参数手动设置。HDR 太阳在 equirect 中占据一定角径，可从中推导 `light_size`。

### 潜在方案

- 从自动太阳检测获得太阳���半径（度）
- 换算为 PCSS `light_size` 参数（世界空间尺度，取决于太阳距离假设）
- 作为默认值填入，用户仍可手动覆盖

---

## Lightmap 改进：烘焙排除太阳 + 更高质量直接光（程序化天空阶段构想，来自阶段八点五 Step 5.1）

当前 lightmap 烘焙了完整 HDR 辐射（含太阳直射），但 lightmap 分辨率（默认 10 texels/meter）不足以表达太阳锐利阴影，导致直射区域质量低于实时方向光。

### 改进方向

程序化天空阶段排除太阳直射后：
- Lightmap 只存间接光（低频信息，当前分辨率��够）
- 方向光通过 CSM 提供高精度直射（含锐利阴影）
- 两者直接相加，物理正确且视觉质量最优

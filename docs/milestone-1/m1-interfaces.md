...
// Per-draw push constant — 72 bytes, within 128-byte minimum guarantee
// 对应 shader: push_constant, stage = VERTEX | FRAGMENT
// 演进计划：阶段六迁移到 per-instance SSBO（push constant 缩减为 {instance_id, cascade_index}），
...
struct PushConstantData {
    ...
    uint32_t cascade_index;                     //  4 bytes — shadow.vert 使用（阶段四新增）
};
```
...
// Per-draw 数据 (72 bytes)
layout(push_constant) uniform PushConstants {
    mat4 model;
    uint material_index;
    uint cascade_index;          // 阶段四新增，shadow.vert 使用
};
```

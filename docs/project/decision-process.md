...
**适用范围**：per-effect temporal 作用于**独立的中间效果纹理**（AO texture、SSR result 等），对每个效果单独降噪。它**不能**平滑嵌在最终光照颜色中的信号噪声（如 shadow cascade dithering、specular aliasing）——那些需要 FSR/DLSS 的**全画面 temporal accumulation**（对最终颜色做时域积累，SDK 自带实现，依赖 Motion Vectors，不复用 per-effect 基础设施）。
...
| per-effect temporal filtering | → SSAO/SSGI/SSR 受益 | 共享 per-effect 历史 buffer + reprojection，仅作用于独立中间效果纹理，不覆盖最终颜色中的噪声（如 shadow dithering） |
| 全画面 temporal（FSR/DLSS） | → shadow dithering、specular aliasing 等最终颜色噪声 | SDK 自带实现，对最终颜色做时域积累，依赖 Motion Vectors，与 per-effect temporal 是独立系统 |

...
| `app/include/himalaya/app/scene_loader.h` | 修改：新增 pending 数据 + `prepare_uv_requests()` / `apply_lightmap_uvs()` 方法 |
...
scene_loader_.apply_lightmap_uvs();  // 全量重建 VB/IB（缓存命中秒过）
ctx_.end_immediate();
ctx_.begin_immediate();
...
- `app/debug_ui.cpp`：新增 Baking collapsing header（始终显示，默认折叠）：
  ...
  - **烘焙触发**：Start Bake 按钮（仅 rt_supported + 有场景 + 有 HDR + RenderMode != Baking + IBL 模式时可用）。按钮旁显示当前 IBL 旋转角度（round 后整数度），tooltip 说明"将以此角度 bake，结果按角度缓存"。点击后始终：`cancel generator → apply_lightmap_uvs → build_scene_rt → start_bake`，切换 RenderMode 到 Baking（进入 Baking 模式是唯一入口，不通过 RenderMode combo）

...
| 文件 | 动作 |
|------|------|
| `framework/include/himalaya/framework/lightmap_uv.h` | 修改：新增 `LightmapUVQuality` 枚举 + `kDefaultLightmapUVQuality` 编译期常量（签名不变） |
...
| `app/include/himalaya/app/scene_loader.h` | 修改：新增 pending 数据 + `prepare_uv_requests()` / `apply_lightmap_uvs()` / `has_pending_uvs()` 方法 |
...
`generate_lightmap_uv()` 签名不变，内部使用 `kDefaultLightmapUVQuality` 决定 xatlas 参数和缓存 key。Quality 是编译期常量，不需要作为运行时参数传递。

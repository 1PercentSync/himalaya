...
- DebugUI 扩展：split lambda 滑条、shadow max distance 滑条
...
关键设计：
- `shadow_max_distance`（ShadowConfig 参数）限制 cascade 覆盖范围，不一定等于相机 far plane，避免远距离浪费分辨率
...
- Debug render mode 新增 cascade index 可视化（每个 cascade 用不同颜色标注），添加到 DebugUI 渲染模式下拉列表
...
- cascade 可视化：`debug_render_mode` 枚举追加新值，forward.frag 根据 `select_cascade()` 返回的 cascade index 输出对应颜色
...
- Cascade blend：相邻 cascade 的 blend region 内线性插值两个 cascade 的 shadow 值
- Distance fade：shadow max distance 边缘渐变到无阴影（1.0）
- Normal offset bias 与 PCF 配合调参
- Culling 模块泛化：`framework/culling.h` 新增通用 frustum 剔除函数（接受任意 6 平面 frustum），ShadowPass 每个 cascade 调用做光空间剔除，替代暴力全画
...
PCF 和 cascade blend 见 `milestone-1/m1-design-decisions.md`「CSM 阴影系统」。剔除泛化见「CSM 阴影系统 — Per-cascade 剔除」。
...
- Culling 泛化：从 `cascade_view_proj` 提取正交 frustum 6 平面，调用通用 `cull_against_frustum()` 返回可见索引。ShadowPass 内部 per-cascade 调用。Camera frustum cull 也迁移到通用接口
- M2 FSR/DLSS 接入后 cascade blend 可切换为 dithering（省去 blend region 双重采样），阶段四不实现
...
| Cascade 可视化 | debug_render_mode 追加 cascade index 热力图，归入材质属性模式（跳过 ACES） |

...
Step 10: AO Debug Modes + Temporal UI
    ↓ (完整 AO 管线，近似 SO 基线)
Step 10a: GTAO 正确性修复（horizon search 重构 + thickness heuristic）
    ↓
Step 10b: GTAO 质量增强（采样优化 + 空间降噪 pass）
    ↓
Step 10c: HDR Sun 方向光 + 色温模式（app 层，不影响渲染管线）
    ↓
...
| 10a | GTAO 正确性修复 | 深度不连续处无黑色光晕，倾斜表面 AO 强度正确 |
| 10b | GTAO 质量增强 | banding 消除，空间降噪后输出干净，temporal 积累有效 |
| 10c | HDR Sun 方向光 + 色温模式 | HDR Sun 模式光方向随 IBL 旋转同步，色温滑条可调，坐标持久化生效 |
| 11 | Roughness Buffer | RenderDoc: roughness 值与材质一致 |
| 12 | Bent Normal + GTSO | 对比 Step 9 基线，SO 更精确 |
...
关键设计：
- GTAO 输出格式：Step 7~11 使用 RG8（R=AO, G=0），Step 12 升级为 RGBA8（RGB=bent normal, A=AO）
- ao_noisy / ao_blurred / ao_filtered 在 Step 12 从 RG8 升级为 RGBA8（bent normal 需要 3 个通道）
...
- SO 不在此步实现——先验证 diffuse AO 正确性，Step 12 升级到 GTSO（bent normal）
...
实现细节：
- G 通道当前为 0，passthrough 处理（Step 12 升级 GTSO 后改为 RGBA8，bent normal RGB blend 不做邻域 clamp，使用与 AO 相同的 blend factor）
...
- Specular Occlusion：Lagarde 近似公式（`saturate(pow(NdotV + ao, exp2(-16 * roughness - 1)) - 1 + ao)`，Lagarde & de Rousiers 2014），调制 IBL specular
...
实现细节：
- 此步使用 Lagarde 近似公式（`saturate(pow(NdotV + ao, exp2(-16 * roughness - 1)) - 1 + ao)`，Lagarde & de Rousiers 2014）作为 SO 中间验证基线，不需要 roughness buffer
- Step 12 升级到 GTSO 后，近似公式删除，改为读 AO 纹理 RGB（bent normal）+ A（AO），解析公式计算 cone 交集 SO
...
修复 GTAO 算法的数学正确性问题和深度不连续处的黑色光晕。核心改动在 `gtao.comp`，C++ 侧仅 push constants 新增 `frame_index`。

- 重构 horizon search 流程：法线投影从搜索循环后移到循环前（每 slice 先投影法线再搜索），使初始化和 falloff 可使用正确的切平面极限
- 修复 `n_proj_len` 缺失：slice 可见性乘以投影法线长度（GTAO 论文公式要求）
- Horizon 初始化改为 `cos(γ ± π/2)` 切平面极限，替代固定 bias
- Falloff 目标改为切平面极限（而非固定 bias），使衰减行为与视角无关
- 添加 thickness heuristic：后续样本深度回退时衰减 horizon（解决薄物体光晕）
- Push constants 新增 `frame_index`，IGN 噪声加入帧间变化（temporal 可跨帧积累更多方向）

**验证**：深度不连续处（前景物体轮廓 vs 背景墙面）无可见黑色光晕；倾斜表面 AO 强度正确（对比修复前）；temporal 帧间积累有效（相机静止时 AO 逐帧收敛）

#### 设计要点

见 `milestone-1/m1-phase5-decisions.md`「GTAO 正确性修订」。
...
- 步进分布改为二次幂曲线（`(j/N)²`，样本集中在像素附近的细缝/角落处）
- 添加 R1 序列步进抖动（消除步进量化 banding）
- Falloff 形状调整：flat inner + steep outer（内 ~38% 半径全权重，外 ~62% 线性衰减）
- 新增 `ao_spatial.comp`：5×5 edge-aware bilateral blur（深度差异驱动边缘权重，不跨越深度不连续处模糊）
- 新增 `AOSpatialPass` 类（setup / record / destroy / rebuild_pipelines），Set 3 push descriptor layout
- Renderer 编排：GTAO → AO Spatial Blur → AO Temporal 管线串联

**验证**：banding artifacts 消除（对比 10a）；空间降噪后输出干净（深度边缘锐利、平坦区域平滑）；temporal 积累更有效（R1 步进抖动 + 帧间噪声变化提供更多独立样本）

#### 设计要点

见 `milestone-1/m1-phase5-decisions.md`「AO 空间降噪」。

---

### Step 10c：HDR Sun 方向光 + 色温模式

App 层功能增强，不影响渲染管线。新增基于 HDR 太阳坐标的方向光模式，以及 Fallback / HDR Sun 两种模式的色温控制。

- `color_temperature_to_rgb(float kelvin)` 工具函数（`framework/color_utils.h`），Kelvin → 线性 RGB，范围 2000K~12000K
- IBL 类新增 `equirect_width()` / `equirect_height()` getter（`init()` 开头 `stbi_info()` 读 HDR header，不加载像素）
- `LightSourceMode` 枚举新增 `HdrSun`，DebugUI combo 适配四项
- `AppConfig` 新增 `hdr_sun_coords`（`std::unordered_map<std::string, std::pair<int,int>>`），持久化到 `config.json`
- Application 新增 HDR Sun 成员（`hdr_sun_x_`、`hdr_sun_y_`、`hdr_sun_intensity_`、`hdr_sun_color_temp_`、`hdr_sun_cast_shadows_`、`hdr_sun_light_`）
- `update()` 中 HDR Sun 方向计算：equirectangular 像素坐标 → HDR 空间方向 → 逆 `rotate_y`（HDR→世界）→ `DirectionalLight.direction`
- Fallback 模式新增 `fallback_light_color_temp_`（默认 6500K），`fallback_light_.color` 由 `color_temperature_to_rgb()` 计算
- HDR Sun 模式同样使用色温控制光色
- DebugUI：HDR Sun 模式显示 Sun X/Y 输入、Intensity slider、Color Temp slider（2000~12000K）、Cast Shadows checkbox；Fallback 模式新增 Color Temp slider
- 切换 HDR 时自动查找已存坐标，修改坐标时自动保存；无坐标时使用 (0, 0) 默认

**验证**：HDR Sun 模式下光方向与天空盒中太阳位置对齐（旋转 IBL 时方向同步），色温滑条在 Fallback 和 HDR Sun 模式均生效，坐标持久化可跨会话保持

#### 设计要点

见 `milestone-1/m1-phase5-decisions.md`「HDR Sun 方向光」「色温模型」。
...
// HDR 空间方向（指向太阳，IBL 旋转前）
sun_dir = (cos(θ)·cos(φ), sin(θ), cos(θ)·sin(φ))

// 逆 IBL 旋转（shader 做 world→HDR，此处做 HDR→world，取反 sin）
rotated_sun = inverse_rotate_y(sun_dir, sin(ibl_yaw_), cos(ibl_yaw_))

// 方向光 direction（光线传播方向 = 反向）
light.direction = -rotated_sun
```
...
│   ├── ao_spatial_pass.h        # [Step 10b] AO spatial blur compute pass
...
└── src/
    ...
    ├── ao_spatial_pass.cpp      # [Step 10b]
    ├── ao_temporal_pass.cpp     # [Step 8]
    └── contact_shadows_pass.cpp # [Step 13]
framework/
├── include/himalaya/framework/
│   └── color_utils.h            # [Step 10c] 色温 → 线性 RGB 转换
└── src/
    └── color_utils.cpp          # [Step 10c]
shaders/
├── gtao.comp                   # [Step 7] GTAO, [Step 10a] 正确性修复, [Step 10b] 采样优化, [Step 12] + bent normal
├── ao_spatial.comp             # [Step 10b] 5×5 edge-aware bilateral blur
├── ao_temporal.comp            # [Step 8] AO temporal filter, [Step 12] RGBA8 适配（bent normal + AO）
...
│   └── ibl.h                   # [Step 10c] equirect_width/height getter
├── src/
...
│   ├── debug_ui.h              # [Step 6] DebugUIContext 扩展, [Step 10c] LightSourceMode::HdrSun + 色温字段
│   ├── application.h           # [Step 10c] HDR Sun 成员 + fallback 色温成员
│   └── config.h                # [Step 10c] AppConfig hdr_sun_coords
├── src/
│   ├── renderer.cpp            # [Step 2-14]
│   ├── application.cpp         # [Step 4] config 初始化 + RenderFeatures, [Step 10c] HDR Sun 方向计算 + 色温
│   ├── config.cpp              # [Step 10c] hdr_sun_coords 序列化/反序列化
│   └── debug_ui.cpp            # [Step 6-14] 面板逐步填充, [Step 10c] HDR Sun/Fallback 色温 UI
shaders/
├── common/bindings.glsl        # [Step 4] FEATURE_AO, FEATURE_CONTACT_SHADOWS, GlobalUBO 新增字段
│                               # [Step 5] Set 2 binding 1-4
├── forward.frag                # [Step 9] AO 采样 + 近似 SO, [Step 12] GTSO (bent normal cone 交集), [Step 14] contact shadow

...
- `apply_lightmap_uvs()` → void
  ...
  - 遍历所有 mesh（不仅是 pending 的）：
    1. 对 pending prim：调用 `generate_lightmap_uv()`（应命中缓存；cache miss 时 warn log 并 fallback 单线程生成），应用 remap + uv1 重建顶点/索引数组，更新 `cpu_vertices_[i]` / `cpu_indices_[i]`
...
- `app/debug_ui.cpp`：新增 Baking collapsing header（始终显示，默认折叠）：
  ...
  - **烘焙触发**：Start Bake 按钮（仅 rt_supported + 有场景 + 有 HDR + RenderMode != Baking + IBL 模式时可用）。按钮旁显示当前 IBL 旋转角度（round 后整数度），tooltip 说明"将以此角度 bake，结果按角度缓存"。点击后始终：`ensure generator running → wait → apply_lightmap_uvs → build_scene_rt → start_bake`，切换 RenderMode 到 Baking（进入 Baking 模式是唯一入口，不通过 RenderMode combo）

...
| 6 | Push Constants 扩展 | RT shader 编译通过（push constant 64B 超集），reference view 行为不变 |
...
- 新增 `app/renderer_bake.cpp`：
  ...
  - `render_baking()` 状态机：
    - `BakingLightmaps`：当前 instance 烘焙中 → 每帧调用 lightmap_baker_pass_.record() → 达到目标采样数后 → GPU idle → readback accumulation + aux albedo + aux normal → BakeDenoiser::denoise()（含辅助通道）→ upload denoised → compress_bc6h() → readback BC6H → write_ktx2() → 释放资源 → 下一个 instance
...
- 新增 `passes/probe_baker_pass.h/.cpp`：
  ...
  - `record()`：RG pass 注册 accumulation cubemap ReadWrite + aux albedo/normal Write + push descriptors + trace_rays(face_w, face_h)（dispatch 一次覆盖一个 face，6 次 dispatch 或 launch_size.z = 6）
  - Set 3 layout：binding 0 accumulation cubemap（image2DArray storage）+ binding 1 aux albedo (storage, per-face 分辨率) + binding 2 aux normal (storage, per-face 分辨率) + binding 3 Sobol SSBO

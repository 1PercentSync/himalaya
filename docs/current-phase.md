...
- `apply_lightmap_uvs()` → void
  ...
  - 遍历所有 mesh（不仅是 pending 的）：
    1. 对 pending prim：调用 `generate_lightmap_uv()`（应命中缓存），检查返回的 `cache_hit` 标记，miss 时 `spdlog::warn` 并 fallback 单线程生成。应用 remap + uv1 重建顶点/索引数组，更新 `cpu_vertices_[i]` / `cpu_indices_[i]`

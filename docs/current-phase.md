...
Step 1: GPUInstanceData 清理 + probe_count 引入（基础设施重构）
    ↓
Step 2: Manifest grid_spacing + BakeDataManager 读写更新
    ↓
Step 3: 3D Grid 空间索引（加载时构建，依赖 Step 2 的 grid_spacing）
    ↓
...
| 2 | Manifest grid_spacing | manifest 写入/读取含 `grid_spacing`，`scan()` 和 `load_angle()` 正确解析新格式 |
...
Manifest 格式引入 `grid_spacing` 字段，为 Step 3 的 3D Grid 构建提供 cell size。

#### 2a. Manifest 格式变更

- 当前格式：`uint32_t probe_count` + `vec3[probe_count] positions`
- 新格式：`uint32_t probe_count` + `float grid_spacing` + `vec3[probe_count] positions`（header 8B + 12B per probe）
- 旧 bake 数据由用户手动清除后重新 bake

#### 2b. 写入侧

- `renderer_bake.cpp`：`write_probe_manifest()` 在 `probe_count` 后写入 `grid_spacing`（从 `bake_locked_config_.grid_spacing` 获取）

#### 2c. 读取侧

- `bake_data_manager.cpp`：`load_angle()` 从 manifest 读取 `grid_spacing`，存储为成员变量供 3D Grid 构建使用
- `bake_data_manager.cpp`：`scan()` 同步更新解析逻辑（offset 4 = `grid_spacing`，positions 从 offset 8 开始）

**验证**：bake 后 manifest 含 grid_spacing，load_angle 正确读取
...
- `bake_data_manager.h`：新增 `ProbeGrid3D` 内部结构
  ...
  - `float cell_size`（从 manifest 读取的 `grid_spacing`）

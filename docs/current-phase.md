...
- `scripts/generate_poisson_disk.py`：dart-throwing 算法生成单位圆内 Poisson Disk 样本点
  - 输入：样本数（16 / 25）、最小间距（由样本数自动推算：`0.7 / sqrt(N)`）、随机种子（固定值，保证可复现）
  - 算法：在单位圆内均匀随机采样候选点，拒绝与已有点距离小于最小间距的候选，直到达到目标数量
  - 输出：打印为 GLSL `const vec2 kBlockerSearchSamples[16]` / `const vec2 kPCFSamples[25]` 格式，可直接粘贴到 shader
  - 脚本留在仓库中作为坐标的可复现来源

#### Shader：Poisson Disk 采样分布

- `shadow.glsl` 新增由 `scripts/generate_poisson_disk.py` 生成的 Poisson Disk 常量数组：16 个 `vec2` 用于 Blocker Search + 25 个 `vec2` 用于 PCF
- 样本分布在单位圆内，硬编码为 shader 常量（`const vec2 kBlockerSearchSamples[16]`、`const vec2 kPCFSamples[25]`）
...
**已知局限**：cascade 边界处 2×2 quad 内像素可能使用不同 `cascade_view_proj` 矩阵，导致 `dFdx(shadow_uv)` 混合不同投影空间的导数——数学上不精确。但此情况仅影响 1-2 像素宽的窄带，且 receiver plane bias 是 normal offset + slope bias 之上的第三层修正，退化后等价于固定 PCF 路径的 bias 质量，视觉上不可见。
...
- `blend_cascade_shadow()` 根据 `global.shadow_mode` 分支：
  ...
  - `shadow_mode == 1`（PCSS）：在 blend 分支外预计算两个 cascade 的 `ShadowProjData`（保证 `dFdx/dFdy` 在 uniform flow 中执行），然后调用 `sample_shadow_pcss(proj, cascade)`；blend region 内使用预计算的 `next_proj` 采样下一 cascade

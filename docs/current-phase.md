...
AO Spatial Blur (compute)                          ← [Step 10b] 5×5 edge-aware bilateral blur
  ...
  写: ao_blurred (RG8, push descriptor storage image)
    ↓
AO Temporal (compute)
...
shaders/
├── gtao.comp                   # [Step 7] GTAO, [Step 10a] 正确性修复, [Step 10b] 采样优化, [Step 12] + SO
...
| AO 资源 usage | ao_noisy / ao_blurred / ao_filtered / contact_shadow_mask: `Storage \| Sampled`（compute storage write + sampled read）。roughness: `ColorAttachment \| Sampled`（DepthPrePass color attachment + GTAO push descriptor sampled） |

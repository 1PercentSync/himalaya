...
- `init()` 内部分解为 6 个私有方法：`load_equirect()` → `convert_equirect_to_cubemap()` → `compute_irradiance()` → `compute_prefiltered()` → `compute_brdf_lut()` → `register_bindless_resources()`，`init()` 本身仅做编排。equirect 输入 image 作为 `load_equirect()` 返回值传递给 `convert_equirect_to_cubemap()`，不作为类成员——它仅在 init scope 内存活。`ctx`、`sc` 按引用传入各私有方法，不存储；`rm_`、`dm_` 存为成员指针供 `destroy()` 使用
...
- 创建 shader 公共库（constants.glsl、brdf.glsl、lighting.glsl），将 Step 6.5 内联的 IBL 代码重构为 `evaluate_ibl()` 调用
...
关键设计：
- forward.frag 原地升级 Lambert → Cook-Torrance + IBL `evaluate_ibl()` 调用，不保留 Lambert 切换
- 无条件消费全部 5 个材质纹理（bindless default 纹理消除特判）
- Step 6.5 的 IBL 内联代码重构到 `lighting.glsl` 的 `evaluate_ibl()`（逻辑不变，仅架构化）

...
| 文件 | 动作 |
|------|------|
| `framework/include/himalaya/framework/lightmap_uv.h` | 修改：新增 `LightmapUVQuality` 枚举 + `generate_lightmap_uv()` 增加 quality 参数 |
| `framework/src/lightmap_uv.cpp` | 修改：quality 分支（Fast/Production xatlas 参数）+ 缓存子目录隔离 |
| `framework/include/himalaya/framework/lightmap_uv_generator.h` | **新建**：`LightmapUVGenerator` 类声明 |
| `framework/src/lightmap_uv_generator.cpp` | **新建**：线程池实现 |
| `framework/CMakeLists.txt` | 修改：添加 `lightmap_uv_generator.cpp` |
| `app/include/himalaya/app/scene_loader.h` | 修改：新增 pending 数据 + `prepare_uv_requests()` / `apply_lightmap_uvs()` / `has_pending_uvs()` |
...
`generate_lightmap_uv()` 签名扩展：

```cpp
LightmapUVResult generate_lightmap_uv(std::span<const Vertex> vertices,
                                      std::span<const uint32_t> indices,
                                      const std::string &mesh_hash,
                                      LightmapUVQuality quality = kDefaultLightmapUVQuality);
```

缓存路径从 `lightmap_uv/{hash}.bin` 改为 `lightmap_uv/fast/{hash}.bin` / `lightmap_uv/production/{hash}.bin`。Debug 和 Release 缓存互不干扰。
...
public:
    ...
    void start(std::vector<Request> requests, LightmapUVQuality quality,
...
while (true) {
    ...
    generate_lightmap_uv(requests_[idx].vertices, requests_[idx].indices,
                         requests_[idx].mesh_hash, quality_);
    completed_.fetch_add(1);  // 结果丢弃，只关心缓存写入
}
```

`cancel()` 设标记后 `join` 所有线程（当前正在执行的 mesh 会完成后退出）。`start()` 前若已有线程在跑，先 `cancel()`。
...
3. 记录 prim 索引到 `uv_pending_prims_`，hash 到 `uv_pending_hashes_`
4. 原样上传到 GPU（uv1 全零）
...
- `prepare_uv_requests()` → `std::vector<LightmapUVGenerator::Request>`
  - 遍历 `uv_pending_prims_`，从 `cpu_vertices_[prim]` / `cpu_indices_[prim]` 拷贝数据 + 对应 hash
  - 返回值供 Application 传给 `LightmapUVGenerator::start()`

- `apply_lightmap_uvs(LightmapUVQuality quality)` → void
  - **必须在 immediate scope 内调用**
  - 遍历所有 mesh（不仅是 pending 的）：
    1. 对 pending prim：调用 `generate_lightmap_uv()`（应命中缓存），应用 remap + uv1 重建顶点/索引数组，更新 `cpu_vertices_[i]` / `cpu_indices_[i]`
  ...
  - 清空 `uv_pending_prims_` / `uv_pending_hashes_`

- `has_pending_uvs()` → `!uv_pending_prims_.empty()`
...
**按钮状态规则**：
- **Start**：`!generator.running() && scene_loader.has_pending_uvs()` 时可点击，否则灰显
- **Stop**：`generator.running()` 时可点击，否则灰显

**状态文本**：
- 无场景 / 无 pending：不显示
- Idle + has pending：`"Idle — {total} meshes pending"`
...
bool has_pending_uvs;             // read-only
...
```cpp
if (config_.bg_uv_auto_start && scene_loader_.has_pending_uvs()) {
    resolve_thread_count();  // 若 bg_uv_thread_count == 0 则解析并持久化
    auto requests = scene_loader_.prepare_uv_requests();
    uv_generator_.start(std::move(requests), kDefaultLightmapUVQuality,
...
**点击 Bake（Start Bake 按钮响应）**：

```cpp
uv_generator_.cancel();  // 停止后台生成（若在运行）
if (scene_loader_.has_pending_uvs()) {
    ctx_.begin_immediate();
    scene_loader_.apply_lightmap_uvs(kDefaultLightmapUVQuality);
    ctx_.end_immediate();
    // 重建 BLAS/TLAS（新 VB/IB handle）
    ctx_.begin_immediate();
...
- `app/debug_ui.cpp`：新增 Baking collapsing header（始终显示，默认折叠）：
  ...
  - **烘焙触发**：Start Bake 按钮（仅 rt_supported + 有场景 + 有 HDR + RenderMode != Baking + IBL 模式时可用）。按钮旁显示当前 IBL 旋转角度（round 后整数度），tooltip 说明"将以此角度 bake，结果按角度缓存"。点击后：`cancel generator → apply_lightmap_uvs（若 has_pending_uvs）→ build_scene_rt → start_bake`，切换 RenderMode 到 Baking（进入 Baking 模式是唯一入口，不通过 RenderMode combo）

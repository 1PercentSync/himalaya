...
- Workgroup size 8×8（每像素独立计算，无 shared memory 需求）
...
- Material AO 只调制 diffuse indirect（`ssao × material_ao` + multi-bounce），不参与 specular（AO 是方向无关标量，specular 是方向相关的，标量 AO 乘 specular 会错误压暗反射方向朝向开阔区域的表面）
- Specular indirect 完全由 SO 控制：此步使用 Lagarde 近似公式（业界广泛使用：Unity HDRP、UE 等），基于材质 roughness + NdotV + AO 标量推导 SO，不需要 roughness buffer
...
| AO 集成 | Diffuse indirect: `ssao × material_ao` + multi-bounce 色彩补偿 (Jimenez 2016)。Specular indirect: 仅由 SO 控制（Step 9 Lagarde 近似，Step 12 读 G 通道），material AO 不参与 specular（标量 AO 乘方向相关的 specular 物理不正确） |
| Multi-bounce | `max(vec3(ao), ((ao * a + b) * ao + c) * ao)` 其中 a/b/c 由 albedo 决定。浅色表面压暗减轻 |
| 无 blur pass | GTAO + temporal 即可得到干净结果，不需要额外空间模糊 |
| RG temporal | RG managed image 标记 `temporal=true` → 内部 double buffer，`clear()` 自动 swap，resize 重建两张。`get_history_image()` 始终返回 valid RGResourceId，`is_history_valid()` 查询有效性。`use_managed_image(handle, final_layout)` 调用方显式传 final_layout（temporal current 传 SHADER_READ_ONLY_OPTIMAL，非 temporal 传 UNDEFINED），RG 不区分 temporal/非 temporal |

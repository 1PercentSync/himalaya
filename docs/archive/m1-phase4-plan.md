...
- `GlobalUniformData` 新增字段（656 → 720 bytes）：
  ...
  - `vec4 cascade_light_size_uv`：per-cascade `LIGHT_SIZE_UV = 2 * tan(θ/2) / width_x`（CPU 预计算，光源角尺寸在 UV 空间的投影）
...
- `shadow.glsl` 新增 `blocker_search(ShadowProjData proj, int cascade, out float avg_blocker_depth, out float num_blockers)`：
  ...
  - 搜索半径 U = `max(cascade_light_size_uv[cascade], kMaxPenumbraTexels * shadow_texel_size)`，V = U × `cascade_uv_scale_y[cascade]`（椭圆搜索）。取 max 确保搜索范围至少覆盖最大半影宽度，防止半影外缘因搜索不到 blocker 而产生硬切边
...
- `shadow.glsl` 新增 `sample_shadow_pcss(ShadowProjData proj, int cascade)`：
  ...
  - Blocker confidence fade：当 blocker 数量极少时（搜索边界处），通过 `smoothstep(0, 4, num_blockers)` 将 PCF 结果渐变混合向 1.0（全亮），平滑半影最外缘。与搜索半径扩展配合——扩展后边界处能找到少量 blocker，confidence fade 让结果渐入而非硬切
...
| PCSS 方向光简化 | 正交投影下 `LIGHT_SIZE_UV` 为常数（无透视缩放），半影公式去掉 `1/dBlocker` 除法。Blocker search 实际搜索半径取 `max(LIGHT_SIZE_UV, kMaxPenumbraTexels × texel_size)` 以覆盖最大半影范围 |
...
| PCSS Blocker Confidence Fade | blocker 数量极少时（搜索边界处）`smoothstep(0, 4, num_blockers)` 将 PCF 结果混合向全亮，与搜索半径扩展配合平滑半影最外缘。4 个 blocker 以上 confidence = 1.0（无衰减） |

...
- **Phase**：阶段四 — 阴影
- **进度**：准备工作 B（Instancing）完成，准备工作 C（运行时加载）进行中（代码完成，待验证），准备工作 D 构建配置和文档已完成（xxHash vcpkg + 自写 KTX2 方案确定）

### 下一个任务

准备工作 C 第十小项（验证）：运行时切换 scene/HDR 正常，配置持久化，重启后恢复上次文件，文件丢失时 fallback 正确。

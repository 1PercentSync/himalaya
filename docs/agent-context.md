...
- **进度**：Step 5（Tile Binning）已完成 `gs_tile_count.comp` 与 `gs_tile_scan.comp`，待继续 tile scatter

### 下一个任务

Step 5：创建 `shaders/gs/gs_tile_scatter.comp`（再次遍历可见 splat，按 offset + atomicAdd 写入 tile_splat_ids）

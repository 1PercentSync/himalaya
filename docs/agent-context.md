...
- **进度**：Step 3 进行中（descriptor 系统全部完成，进入 Compute Infra，checkbox 1-8 done）

### 下一个任务

阶段三 Step 3：Descriptor Layout + Compute Infra — `pipeline.h` 新增 `ComputePipelineDesc` + `create_compute_pipeline()`。

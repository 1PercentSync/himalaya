...
Step 5 第 4 项：`Renderer::handle_shadow_resolution_changed(uint32_t new_resolution)` — `vkQueueWaitIdle` → `shadow_pass_.on_resolution_changed()` 重建 image + views → 更新 Set 2 binding 5

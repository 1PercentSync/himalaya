...
- **Phase**：阶段四 — 阴影
- **进度**：准备工作 A/B/C 完成，D-1 全部完成，D-2 代码全部完成 + 审查修复完成（sRGB mip 修正、线程安全、源文件 hash 缓存优化、KTX2 模块重构），待验证

### 下一个任务

准备工作 D-2 验证：纹理 VRAM 显著降低（RGBA8 → BC 约 4:1），缓存命中时跳过解码和压缩，画质无明显退化。验证前需清除旧缓存（`%TEMP%\himalaya\textures\`）。

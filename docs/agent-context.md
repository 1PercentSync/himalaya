...
- **进度**：Step 3 进行中（模块删除 + scene_data.h 类型/PTConfig/RenderFeatures/RenderMode 完成），下一项精简 GlobalUniformData

### 下一个任务

Step 3：清理 `scene_data.h` — 精简 GlobalUniformData（移除 shadow cascade、AO、probe blend、directional_light_count 字段）

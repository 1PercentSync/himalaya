...
- [ ] 类型提取：`BakeState` 迁移到 `scene_data.h`（与 `RenderMode` 一致）+ 新增 `framework/render_progress.h`（`BakeProgress`）+ `renderer.h` 去除嵌套类型改为 include + `renderer_bake.cpp` 更新所有 `Renderer::BakeState` / `Renderer::BakeProgress` 引用

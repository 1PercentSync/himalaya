...
- [ ] `scene_loader.cpp`：`apply_lightmap_uvs()` 实现（全量重建 VB/IB：pending 走 xatlas 缓存，其余原样，需 immediate scope，不清空 pending）
...
- [ ] `application.cpp`：Start Bake 流程前置步骤（始终：cancel generator → apply_lightmap_uvs → build_scene_rt → start_bake）
...
- [ ] `debug_ui.cpp`：Start Bake 按钮（唯一入口，旁显当前角度 + tooltip，点击始终触发 cancel generator → apply UV → rebuild RT → start_bake）+ Cancel 按钮（恢复原 RenderMode + 显示取消信息）

...
- [ ] 按 PT 模式拆分 GS Set 3 生命周期职责（descriptor layout 属于 renderer-lifetime pass/pipeline owner；GS scene resource owner 只管理 scene buffers、descriptor set allocation/write/rewrite，不使用 `shutdown()` 式双重销毁语义）

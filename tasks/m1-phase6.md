...
- [ ] closesthit.rchit：propagate cone + 估算曲率修正 spread（face_normal vs interpolated_normal）+ compute per-triangle `base_lod` + per-texture LOD（`textureSize()` 取分辨率）+ LOD clamp 到 `lod_max_level`（push constant）+ 材质纹理 `texture()` → `textureLod()`（~4 处，NEE emissive 光源纹理保持 `texture()`）

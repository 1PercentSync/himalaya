...
- [x] `forward.frag` 重构：移除内联 `get_shading_normal()`，改用 `#include "common/normal.glsl"` 的共享版本（代码审查发现与 depth_prepass 不一致）；采样 `metallic_roughness_tex`，metallic 工作流分离（F0 / diffuse_color）
- [x] `forward.frag` IBL 漫反射 + 镜面反射（irradiance + prefiltered + BRDF LUT Split-Sum，含 `rotate_y`），用 `ibl_intensity` 调制，移除固定 ambient 项
- [x] 灯光体系重构：`ambient_intensity` → `ibl_intensity` 全通路重命名（GlobalUniformData、bindings.glsl、RenderInput、Application、DebugUI）+ 退役 default light（移除 `default_lights_`、`light_pitch_`、`light_intensity_`、`force_default_light_` 及相关逻辑）+ 新增 `disable_scene_lights_`（DebugUI "Disable Scene Lights" checkbox + RenderInput 通路）+ DebugUI Lighting 面板全面更新
- [x] 验证：物体表面 IBL 光照正确（金属面反射环境、粗糙面模糊反射、非金属环境漫反射），`ibl_intensity` 滑条可调，IBL 可旋转，glTF 方向光可通过 checkbox 切换
...
- [x] ~~创建 `shaders/common/lighting.glsl`~~ — 已创建后删除：debug 渲染模式需分离 diffuse/specular 贡献，组合函数无法拆分，forward.frag 改为直接 include brdf.glsl 内联计算
- [x] 升级 `forward.frag`：Lambert 直射光 → Cook-Torrance（GGX / Smith Height-Correlated / Schlick），内联 BRDF 分离 diffuse/specular + 新增 occlusion_tex 调制 IBL + emissive_tex × emissive_factor

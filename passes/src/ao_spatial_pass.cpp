/**
 * @file ao_spatial_pass.cpp
 * @brief AOSpatialPass implementation — pipeline creation and per-frame compute dispatch.
 */
...
namespace himalaya::passes {
    // ---- Workgroup size (must match ao_spatial.comp local_size) ----

    constexpr uint32_t kWorkgroupSize = 8;

    // ---- Init / Destroy ----

    void AOSpatialPass::setup(rhi::Context &ctx,
        ...
        //   binding 0 = storage image (ao_blurred output)
    ...
    void AOSpatialPass::rebuild_pipelines() {
        create_pipeline();
    }

    void AOSpatialPass::destroy() {
    ...
    void AOSpatialPass::create_pipeline() {
        const auto spirv = sc_->compile_from_file("ao_spatial.comp", rhi::ShaderStage::Compute);
        if (spirv.empty()) {
            spdlog::warn("AOSpatialPass: shader compilation failed, keeping previous pipeline");
            return;
        }
    ...
    void AOSpatialPass::record(framework::RenderGraph &rg,
                               const framework::FrameContext &ctx) const {
        // Declare resource usage: read depth + ao_noisy, write ao_blurred
        ...
        rg.add_pass("AO Spatial", resources,
                     [this, &rg, &ctx](const rhi::CommandBuffer &cmd) {
                         ...
                         // Push Set 3 binding 0: ao_blurred storage image output
                         const auto ao_blurred_handle = rg.get_image(ctx.ao_blurred);
                         cmd.push_storage_image(*rm_, pipeline_.layout, 3, 0, ao_blurred_handle);
                         ...
                         const auto &ao_image = rm_->get_image(ao_blurred_handle);

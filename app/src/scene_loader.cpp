...
namespace himalaya::app {
    ...
    bool SceneLoader::load(const std::string &path,
        ...
        try {
            ...
            auto asset = gltf_utils::parse_gltf(file_path, options);
            ...
            load_materials(gltf, file_path.parent_path().string(), material_system, default_textures, default_sampler);
            ...
            [&](fastgltf::Node &node, const fastgltf::math::fmat4x4 &world_transform) {
                ...
                for (uint32_t i = prim_start; i < prim_end; ++i) {
                    mesh_instances_.push_back({
                        ...
                        .world_bounds = gltf_utils::transform_aabb(mesh_data.local_bounds[i], world_mat),
                    });
                }
            });

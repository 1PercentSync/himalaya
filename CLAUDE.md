...
- **禁止**凭记忆断言项目规划（如某功能属于哪个 Milestone/Phase），必须先查 `docs/` 下的文档确认后再陈述
- 只有在确有需要时才使用 `git status` 和 `git diff`
- 已知由自己修改的文件范围时，不得反复用 `git status` / 全仓库 `git diff` 做确认；应基于上下文直接操作明确文件列表
- 不做例行 Git 检查；只有出现明确风险信号（如编辑器/工具提示空白异常、补丁应用失败、用户要求检查）时，才针对具体文件运行最小必要检查
- 非必要不使用 `git diff --stat`

...
2. 用户确认后**提交 commit**（代码和文档分开提交）
3. 提交后**更新 `tasks/`** 中对应任务项状态（勾选 `[x]`），作为单独的 `docs` commit
4. 不要求每一小项都能编译，只需一个 Step 结束时编译通过即可

### 上下文管理

项目计划前置且文档齐全，不依赖 Plan 模式做规划。当用户说「清空上下文」时：
1. 进入 Plan 模式
2. 编写一份简要交接 plan，包含：
   - 当前进度概述
   - 下一步任务
   - 下一步开始前需要阅读的文档列表（含具体路径）
   - 未解决的问题或重要上下文（如有）
3. 退出 Plan 模式，清空对话上下文
4. 新一轮 Agent 通过 plan 文件快速定位并阅读所需文档，恢复工作状态
